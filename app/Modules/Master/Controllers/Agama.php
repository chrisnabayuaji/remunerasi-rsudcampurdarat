...
use App\Modules\Master\Models\AgamaModel;
...
class Agama extends BaseController
{
  ...
  {
    $d['main'] = DB::table('mst_agama')->where('agama_id', $id)->first();
    if ($d['main']) {
  ...
  function save($id = null)
  {
    ...
    try {
      if ($id) {
        DB::table('mst_agama')->where('agama_id', $id)->update($d);
        $msg = 'Data agama berhasil diperbarui';
      } else {
        DB::table('mst_agama')->insert($d);
  ...
  function delete($id, $token)
  {
    ...
    DbModel::beginTransaction();
    try {
      DB::table('mst_agama')->where('agama_id', $id)->delete();
      DbModel::commitTransaction();
      return fsResponse(true, 'Data agama berhasil dihapus', $this->nav_url . '?n=' . $this->nav_id);
    } catch (\Exception $e) {
      DbModel::rollbackTransaction();
      return fsResponse(false, $e->getMessage());
    }
  ...
  {
    return AgamaModel::loadDatatables();
  }
  ...
  function sync()
  {
    ...
    try {
      ...
      $simrsData = DB::connection('simrs')->table('mst_agama')->get();

      if ($simrsData->isEmpty()) {
        DB::rollBack();
        return response()->json([
          'success' => false,
          'message' => 'Tidak ada data agama di SIMRS'
        ]);
      }
      ...
      foreach ($simrsData as $record) {
        try {
          $simrsIds[] = $record->agama_id;

          $exists = DB::table('mst_agama')->where('agama_id', $record->agama_id)->first();

          $data = [
            'agama_id' => $record->agama_id,
            'agama_nm' => $record->agama_nm,
          ...
          if ($exists) {
            DB::table('mst_agama')->where('agama_id', $record->agama_id)->update($data);
            $stats['updated']++;
          } else {
            DB::table('mst_agama')->insert($data);
            $stats['inserted']++;
          }
        } catch (\Exception $e) {
          $stats['errors']++;
          $stats['error_details'][] = "agama_id: {$record->agama_id} - " . $e->getMessage();
          Log::error("Error syncing agama {$record->agama_id}: " . $e->getMessage());
        }
      }

      $placeholders = implode(',', array_fill(0, count($simrsIds), '?'));
      $orphanedData = DB::table('mst_agama')
        ->whereRaw("agama_id NOT IN ($placeholders)", $simrsIds)
        ->where('deleted_st', 0)
        ->get();

      foreach ($orphanedData as $orphan) {
        try {
          DB::table('mst_agama')->where('agama_id', $orphan->agama_id)->update([
            'deleted_st' => 1,
            'active_st' => 0,
          ...
          $stats['errors']++;
          Log::error("Error deleting orphaned agama {$orphan->agama_id}: " . $e->getMessage());
        }
      }
      ...
      DB::rollBack();
      Log::error('Error syncing agama: ' . $e->getMessage());
      return response()->json([
        'success' => false,
